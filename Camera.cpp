...
QMatrix4x4 Camera::get_projection_matrix() {
	QMatrix4x4 projection_matrix;
	if (this->projection_type == "perspective")
		projection_matrix.perspective(projection_params[0], projection_params[1], projection_params[2], projection_params[3]);
	else if (this->projection_type == "ortho")
		projection_matrix.ortho(projection_params[0], projection_params[1], projection_params[2], projection_params[3], projection_params[4], projection_params[5]);
...
void Camera::set_projection_type(QString type) {
	this->projection_type = type;
}

void Camera::set_projection_params(QVector<double> params) {
...
QMatrix4x4 Camera::get_model_matrix()
{
	QVector3D x_eye = (eye - center).normalized() * this->zoom * this->distance_r + center;
	QMatrix4x4 mMat;//模型矩阵取摄像机的位置，避免逃出了天空盒
	mMat.translate(x_eye.x(), x_eye.y(), x_eye.z());
	return mMat;
}

QVector3D Camera::get_pos()
{
	return eye;
}

QVector3D Camera::get_eye()
{
	QVector3D x_eye = (eye - center).normalized() * this->zoom * this->distance_r + center;
	return x_eye;
}
